...
現場投影請開啟 `https://harmonica.nycu.club/p/screen/`，觀眾掃描畫面上的固定 QR Code 後會進入 `https://harmonica.nycu.club/p/`。QR Code 不會隨場次改變，因此可長期放在投影片與實體文宣。

- 按 `F` 或右下角按鈕進入全螢幕。
- 按數字鍵 `1` 至 `4` 標示目前曲目，按 `0` 取消；左右方向鍵也能依序切換。
- 投影網址可直接加上 `?song=2`，開啟時就標示第 2 首。這只影響目前這台電腦的投影畫面，不會改動網站資料。
- 換場次時請網站管理員更新 `data/portal.yaml` 的公開文案與曲目，完成一般 GitHub Pages 部署即可。
- 投影前務必用現場網路實際掃描一次 QR Code，並關閉作業系統通知，避免個人訊息出現在投影上。
